...
export function Countdown() {
  ...
  return (
    <div className="mb-5">
      <p className="mb-2 text-[0.65rem] font-semibold uppercase tracking-[0.14em] text-accent-muted">
        ...
        {units.map((unit, i) => (
          <div key={unit.key} className="flex items-end gap-1 sm:gap-3">
            <div className="flex w-[2.6rem] flex-col items-center sm:w-[2.85rem]">
              <span className="font-mono text-[1.15rem] font-bold tabular-nums leading-none text-[#6b7289] sm:text-[1.25rem]">
                {pad2(parts[unit.key])}
              </span>
            ...
            {i < units.length - 1 ? (
              <span className="mb-[1.05rem] text-[0.85rem] font-semibold text-accent-muted" aria-hidden>
                :
              </span>
            ) : null}
          </div>
